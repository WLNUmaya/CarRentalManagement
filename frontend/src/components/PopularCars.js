...
const PopularCars = ({ data, setAllCars }) => {
    const { id, name, manufacturer, type, capacity, perDay, vehicleType, isBooked } = data;
    ...
    // Function to mark the car as available
    const markCarAsAvailable = async (carId) => {
        try {
            await axios.put(`https://localhost:7009/api/CarDetail/mark-available/${carId}`);
            setAllCars(prevCars => prevCars.map(car => car.id === carId ? { ...car, isBooked: false } : car));
        } catch (error) {
            console.error("There was an error updating the car availability status!", error);
        }
    };
